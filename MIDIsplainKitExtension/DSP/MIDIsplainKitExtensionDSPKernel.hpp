//
//  MIDIsplainKitExtensionDSPKernel.hpp
...
#import "MIDIsplainKitExtension-Swift.h"
#import "MIDIsplainKitExtensionParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 MIDIsplainKitExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class MIDIsplainKitExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in MIDIsplainKitExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case MIDIsplainKitExtensionParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case MIDIsplainKitExtensionParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<MIDIsplainKitExtensionDSPKernel *>(context);
